...
const TypeDetail = () => {
  ...
  const upperType = typeof type === 'string' ? type.toUpperCase() : undefined;
  ...
  const typeData: Record<string, TypeInfo> = {
    ENFJ: {
      ...
      compatibility: {
        excellent: ['ENFP', 'INTJ', 'ENTP'],
        good: ['ISFP', 'INFJ', 'ISFJ', 'ISTJ', 'INFP'],
        challenging: ['ESTJ', 'ESTP', 'ISTP', 'ESFP']
      },
    ...
    INFP: {
      ...
      compatibility: {
        excellent: ['ENFJ', 'ENTJ', 'ENFP', 'INTJ'],
        good: ['INFJ', 'ISFJ', 'ESFJ', 'ENTP'],
        challenging: ['ESTJ', 'ESFP', 'ESTP', 'ISTJ']
      },
  ...
  const currentType = upperType ? typeData[upperType] : null;

  if (!currentType) {
    return (
      <div className="min-h-screen bg-white py-12">
  <Seo title={`タイプが見つかりません | MBTI 一覧`} description={`指定されたMBTIタイプが見つかりません。タイプ一覧から選択してください。`} />
  ...
  return (
    <div className="min-h-screen bg-white py-12">
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="bg-green-50 rounded-2xl p-6 border-2 border-green-200">
              ...
              <div className="flex flex-wrap gap-2">
                {currentType.compatibility.excellent.map((type: string) => (
                  <span key={type} className="px-3 py-1 bg-green-200 text-green-800 rounded-lg text-sm font-medium">
                    {type}
                  </span>
                ))}
              </div>
            ...
            <div className="bg-blue-50 rounded-2xl p-6 border-2 border-blue-200">
              ...
              <div className="flex flex-wrap gap-2">
                {currentType.compatibility.good.map((type: string) => (
                  <span key={type} className="px-3 py-1 bg-blue-200 text-blue-800 rounded-lg text-sm font-medium">
                    {type}
                  </span>
                ))}
              </div>
            ...
            <div className="bg-red-50 rounded-2xl p-6 border-2 border-red-200">
              ...
              <div className="flex flex-wrap gap-2">
                {currentType.compatibility.challenging.map((type: string) => (
                  <span key={type} className="px-3 py-1 bg-red-200 text-red-800 rounded-lg text-sm font-medium">
                    {type}
                  </span>
                ))}
              </div>
        ...
        {/* Pair Anchors for popular compatibility patterns */}
        <div className="mb-12">
